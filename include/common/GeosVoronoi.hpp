...
private:
    void computeDilatedPolygons(std::shared_ptr<WorldState> worldState)
    {
        ...
        for (std::size_t i = 0; i < diagram->getNumGeometries(); ++i) {
            ...
            if (config.dilationOption == DilationOption::JustZero) {
                dilations.push_back(0);
            } else if (config.dilationOption == DilationOption::PuckRadius) {
                dilations.push_back(-config.puckRadius);
            } else if (config.dilationOption == DilationOption::DilationDelta) {
